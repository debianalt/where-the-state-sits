...
#
# Sourced after theme_house.R, which supplies the window (WIN0/WIN1),
...
D2 <- normalizePath(dirname(sys.frame(1)$ofile %||% "."),
                    mustWork = FALSE)
...
# the data layer; run from the repository root they must point back at the data layer's data layer
DIR_PROC <- DIR_PROC2
DIR_RAW  <- DIR_RAW2
...
# from the validated set of theme_house.R.
CLOUD_COL <- "grey74"
MEAN_COL  <- "#3D405B"
MEAN_COL2 <- "#D55E00"
